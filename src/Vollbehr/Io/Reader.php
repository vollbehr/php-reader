...
class Reader
{
    ...
    public function __construct($fd)
    {
        ...
        $size = ftell($this->_fd);
        $this->_size = self::normaliseLength($size === false ? 0 : $size);
        fseek($this->_fd, $offset);
    }

    /**
     * Normalises 32-bit signed integers returned by filesystem functions to unsigned length using floats when needed.
     */
    protected static function normaliseLength(int|float $value): int|float
    {
        if (is_int($value) && $value < 0) {
            if (PHP_INT_SIZE >= 8) {
                return $value & 0xffffffff;
            }
            $unsigned = sprintf('%u', $value);
            if (is_numeric($unsigned)) {
                return (float) $unsigned;
            }
        } elseif (is_float($value) && $value < 0) {
            $intValue = (int) $value;
            if (PHP_INT_SIZE >= 8) {
                return $intValue & 0xffffffff;
            }
            $unsigned = sprintf('%u', $intValue);
            if (is_numeric($unsigned)) {
                return (float) $unsigned;
            }
        }
     ...
     * @return integer
     */
    public function getOffset(): int | float | false
    {
        if ($this->_fd === null) {
        ...
        $offset = ftell($this->_fd);

        return $offset === false ? false : self::normaliseLength($offset);
    }
