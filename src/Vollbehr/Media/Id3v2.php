 ...
 * {@see http://www.id3.org/id3v2.4.0-structure ID3v2 structure document}.
 ...
 */
final class Id3v2 extends Id3\BaseObject
{
    private Id3\Header $_header;

    /** @var Id3\ExtendedHeader */
    ...
    /**
     * Constructs the \Vollbehr\Media\Id3v2 class with given file and options. The
     ...
     *     with. See the documentation of iconv for supported values. Defaults
     *     to ISO-8859-1 for ID3v2.3 tags and UTF-8 for ID3v2.4 unless
     *     explicitly overridden. Please note that write operations do not
     *     convert string and thus encodings are limited to those supported by the {@see \Vollbehr\Media\Id3\Encoding} interface.
    ...
    public function __construct($filename = null, $options = [])
    {
        ...
        $this->normaliseEncodingOption();

        if ($filename === null) {
            $this->_header = new Id3\Header(null, $options);
            $this->normaliseEncodingOption();
        ...
        if ($this->_reader->read(3) != 'ID3') {
            ...
            throw new Id3\Exception('File does not contain ID3v2 tag');
        }

        $this->_header = new Id3\Header($this->_reader, $options);
        $this->normaliseEncodingOption();
        ...
        if ($this->_header->getVersion() < 3 ||
            ...
            throw new Id3\Exception('File does not contain ID3v2 tag of supported version: v2.' .
                 $this->_header->getVersion());
        }
        if ($this->_header->getVersion() < 4 &&
            $this->_header->hasFlag(Id3\Header::UNSYNCHRONISATION)) {
            $data = $this->_reader->read($this->_header->getSize());

            $this->_reader = new \Vollbehr\Io\StringReader($this->decodeUnsynchronisation($data));
            $tagSize       = $this->_reader->getSize();
        }
        $this->clearOption('unsynchronisation');
        if ($this->_header->hasFlag(Id3\Header::UNSYNCHRONISATION)) {
            $this->setOption('unsynchronisation', true);
        }
        if ($this->_header->hasFlag(Id3\Header::EXTENDED_HEADER)) {

            $this->_extendedHeader = new Id3\ExtendedHeader($this->_reader, $options);
        }
        if ($this->_header->hasFlag(Id3\Header::FOOTER)) {
        ...
        while (true) {
            ...
            $firstByte = $this->_reader->read(1);
            if ($firstByte === false) {
                break;
            }

            $firstByteOrd = ord($firstByte);
            if ($firstByteOrd === 0) {
                break;
            }

            $isDigit = $firstByteOrd >= 0x30 && $firstByteOrd <= 0x39;
            $isUpper = $firstByteOrd >= 0x41 && $firstByteOrd <= 0x5a;
            if (!$isDigit && !$isUpper) {
                continue;
            }

            $identifier = $firstByte . $this->_reader->read(3);
            if (!preg_match('/^[A-Z0-9]{4}$/', $identifier)) {
                $this->_reader->setOffset($offset + 1);
            ...
            $classname = sprintf('\\Vollbehr\\Media\\Id3\\Frame\\%s', ucfirst(strtolower($identifier)));
            if (class_exists($classname)) {
                $frame = new $classname($this->_reader, $options);
            } else {
                $frame = new Id3\Frame\Unknown($this->_reader, $options);
            }
     ...
     */
    public function getHeader(): Id3\Header
    {
        return $this->_header;
    }
    ...
    {
        if ($this->_header) {
            return $this->_header->hasFlag(Id3\Header::EXTENDED_HEADER);
        }
     ...
     * otherwise.
     * @return Id3\ExtendedHeader|false
     */
     ...
     * @param Id3\ExtendedHeader $extendedHeader The header object
     */
    public function setExtendedHeader($extendedHeader): void
    {
        if (is_subclass_of($extendedHeader, Id3\ExtendedHeader::class)) {
            $this->_header->flags |= Id3\Header::EXTENDED_HEADER;
            $this->_extendedHeader->setOptions($this->getOptions());
            $this->_extendedHeader = $extendedHeader;
        } else {

            throw new Id3\Exception('Invalid argument');
        }
    }
     ...
     */
    public function hasFrame($identifier): bool
    {
     ...
     * @param string $identifier The frame name.
     */
    public function getFramesByIdentifier($identifier): array
    {
        $matches       = [];
        $searchPattern = '/^' .
     ...
     * @param string $identifier The frame name.
     */
    final public function removeFramesByIdentifier($identifier): void
    {
        $searchPattern = '/^' .
            str_replace(['*', '?'], ['.*', '.'], $identifier) . '$/i';
        foreach ($this->_frames as $identifier => $frames) {
     ...
     * @param Id3\Frame $frame The frame to add.
     * @return Id3\Frame
     */
    public function addFrame($frame)
    {
        ...
        if (is_object($frame) && method_exists($frame, 'synchronizeEncodingWithTag')) {
            $frame->synchronizeEncodingWithTag();
        }
     ...
     * @param Id3\Frame $frame The frame to remove.
     */
    ...
    {
        return $this->_header->hasFlag(Id3\Header::FOOTER);
    }

    /**
     * Returns the footer object if present, or <var>false</var> otherwise.
     * @return Id3\Header|false
     */
     ...
     */
    public function setFooter($useFooter): void
    {
        if ($useFooter) {
            $this->_header->setFlags($this->_header->getFlags() | Id3\Header::FOOTER);
            $this->_footer = &$this->_header;
        } else {
     ...
     * @param string|\Vollbehr\Io\Writer $filename The optional path to the file, use
     *                                        null to save to the same file.
     */
    public function write($filename): void
    {
        if ($filename === null && ($filename = $this->_filename) === null) {
            throw new Id3\Exception('No file given to write to');
        } elseif ($filename !== null && $filename instanceof \Vollbehr\Io\Writer) {
            ...
            throw new Id3\Exception('Unable to copy source to destination: ' .
                 realpath($this->_filename) . '->' . realpath($filename));
        }

        if (($fd = fopen($filename, file_exists($filename) ? 'r+b' : 'wb')) === false) {

            throw new Id3\Exception('Unable to open file for writing: ' . $filename);
        }

        $hasNoFrames = true;
        foreach ($this->_frames as $instances) {
        ...
        if ($hasNoFrames) {
            self::remove(new \Vollbehr\Io\Reader($fd));
        ...
        if ($this->_reader !== null) {
            ...
            $reader = new \Vollbehr\Io\Reader($fd);
            if ($reader->read(3) == 'ID3') {
                $header     = new Id3\Header($reader);
        ...
        $tag = new \Vollbehr\Io\StringWriter();
        $this->_writeData($tag);
        $tagSize = $tag->getSize();
     ...
     */
    private function _writeData(\Vollbehr\Io\Writer | \Vollbehr\Io\StringWriter $writer): void
    {
        $this->normaliseEncodingOption();
        $this->clearOption('unsynchronisation');
        ...
        $frameDataLength = strlen((string) $frameData);
        $paddingLength   = 0;
        ...
        if ($this->getOption('version', 4) < 4 &&
            ...
            $frameData = $this->encodeUnsynchronisation($frameData);
            if ($len = strlen($frameData) !== $frameDataLength) {
                $frameDataLength = $len;
                $this->_header->setFlags($this->_header->getFlags() |
                     Id3\Header::UNSYNCHRONISATION);
            } else {
                $this->_header->setFlags($this->_header->getFlags() &
                     ~Id3\Header::UNSYNCHRONISATION);
            }
        ...
        if ($this->getOption('version', 4) >= 4) {
            $frameData = str_pad((string) $frameData, $frameDataLength += $paddingLength, "\0");
        }
        ...
        if ($this->hasExtendedHeader()) {
            ...
            if ($this->_extendedHeader->hasFlag(Id3\ExtendedHeader::CRC32)) {
                $crc = crc32((string) $frameData);
                if (($crc & 0x80000000) !== 0) {
            ...
            $extendedHeaderDataLength = strlen((string) $extendedHeaderData);
        }
    ...
    /**
     * Normalises the encoding option so it remains valid for the active tag version.
     */
    private function normaliseEncodingOption(): void
    {
        $options = &$this->getOptions();
        $version = (float) ($options['version'] ?? 4.0);
        $encodingOption = $options['encoding'] ?? null;

        if ($encodingOption === null) {
            $encoding = $version < 4.0 ? Id3\Encoding::ISO88591 : Id3\Encoding::UTF8;
        } else {
            $encoding = $this->translateEncodingToInt($encodingOption);
        }

        if ($version < 4.0 && $encoding !== Id3\Encoding::ISO88591 && $encoding !== Id3\Encoding::UTF16) {
            $encoding = Id3\Encoding::ISO88591;
        }
    ...
    public function __get(string $name): mixed
    {
        ...
        $classname = sprintf('\\Vollbehr\\Media\\Id3\\Frame\\%s', ucfirst($name));
        if (class_exists($classname)) {
            return $this->addFrame(new $classname());
        }

        throw new Id3\Exception('Unknown frame/field: ' . $name);
    }
