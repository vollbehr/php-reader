...
/**
 * The _Metadata Library Object_ lets authors store stream-based,
 ...
 * _{@see \Vollbehr\Media\Asf\BaseObject\Metadata Metadata Object}_, as well as
 ...
 */
final class MetadataLibrary extends \Vollbehr\Media\Asf\BaseObject
{
    /** @var Array */
    ...
    public function __construct($reader = null, &$options = [])
    {
        ...
        for ($i = 0; $i < $descriptionRecordsCount; $i++) {
            $descriptionRecord = ['languageIndex' => $this->_reader->readUInt16LE(),
     ...
     *     {@see LanguageList Language List Object} that identifies the
     *     language of this attribute. If there is no _Language List
     *     Object_ present, this field is zero.
     ...
     * following keys.
     *   o languageIndex -- Specifies the index into the _Language List
     *     Object_ that identifies the language of this attribute. If there
     *     is no _Language List Object_ present, this field is zero.
     *   o streamNumber -- Specifies whether the entry applies to a specific
     *     digital media stream or whether it applies to the whole file. A value
     *     of 0 in this field indicates that it applies to the whole file;
     *     otherwise, the entry applies only to the indicated stream number.
    ...
    public function write($writer): void
    {
        ...
        for ($i = 0; $i < $descriptionRecordsCount; $i++) {
            $descriptionRecordsWriter
                ->writeUInt16LE($this->_descriptionRecords[$i]['languageIndex'])
            ...
            if (is_string($this->_descriptionRecords[$i]['data'])) {
                $chunks = [];
                if (preg_match("/^[\da-f]{8}-[\da-f]{4}-[\da-f]{4}-[\da-f]{4}-[\da-f]{1" .
