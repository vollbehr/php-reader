 ...
 */
final class Comment extends \Vollbehr\Media\Vorbis\Header
{
    /** @var string */
    private $_vendor;

    private ?array $_comments = null;

    private int $_framingFlag = 1;
    ...
    public function __construct($reader, $options = [])
    {
        ...
        for ($i = 0; $i < $userCommentListLength; $i++) {
            [$name, $value] = preg_split('/=/', (string) $this->_reader->read($this->_reader->readUInt32LE()), 2);
            if (!isset($this->_comments[strtoupper($name)])) {
                $this->_comments[strtoupper($name)] = [];
            }
    ...
    {
        if (!empty($this->_comments[strtoupper((string) $name)])) {
            return $this->_comments[strtoupper((string) $name)];
        }
        return [];
    }
    ...
    public function __call($name, $arguments)
    {
        ...
        throw new \Vollbehr\Media\Vorbis\Exception('Unknown field: ' . strtoupper($name));
    }
