 ...
 */
final class ExtendedHeader extends BaseObject
{
    /**
    ...
    public function __construct($reader = null, &$options = [])
    {
        ...
        /* ID3v2.4.0 ExtendedHeader */ else {
            ...
            if ($this->hasFlag(self::CRC32)) {
                $this->_reader->skip(1);
                $this->_crc = $this->_reader->readInt8() * (0xfffffff + 1) +
                    $this->decodeSynchsafe32($this->_reader->readUInt32BE());
            }
     ...
     */
    public function useCrc($useCrc): void
    {
        if ($useCrc) {
    ...
    /**
     * Sets the restrictions byte. See {@see #getRestrictions} for more.
    ...
    public function write($writer): void
    {
        ...
        /* ID3v2.4.0 ExtendedHeader */ else {
            $writer->writeUInt32BE($this->encodeSynchsafe32($this->_size))
                   ->writeInt8(1)
            ...
            if ($this->hasFlag(self::CRC32)) {
                $writer->writeInt8(5)
                       ->writeInt8($this->_crc & 0xf0000000 >> 28 & 0xf /*eq >>> 28*/)
                       ->writeUInt32BE($this->encodeSynchsafe32($this->_crc));
            }
            if ($this->hasFlag(self::RESTRICTED)) {
                $writer->writeInt8(1)
