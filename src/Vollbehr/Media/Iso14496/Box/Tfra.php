...
/**
 * The _Track Fragment Random Access Box_ provides entries that contains the location and the presentation time of
 ...
 * {@see \Vollbehr\Media\Iso14496\Box\Trun}, {@see \Vollbehr\Media\Iso14496\Box\Traf} and {@see \Vollbehr\Media\Iso14496\Box\Trex}
 * shall be set appropriately regardless of the presence of this box.
 * @author Georges-Etienne Legendre
 * @author Sven Vollbehr
 */
final class Tfra extends \Vollbehr\Media\Iso14496\FullBox
{
    /** @var integer */
    private $_trackId;
    /** @var Array */
     ...
     *     defined in the {@see \Vollbehr\Media\Iso14496\Box\Mdhd} of the associated track.
     *   o moofOffset: is 32 or 64 bits integer that gives the offset of the {@see \Vollbehr\Media\Iso14496\Box\Moof} used
     *     in this entry. Offset is the byte-offset between the beginning of the file and the beginning of the
     *     {@see \Vollbehr\Media\Iso14496\Box\Moof}.
     *   o trafNumber: indicates the {@see \Vollbehr\Media\Iso14496\Box\Traf} number that contains the random accessible
     *     sample. The number ranges from 1 (the first traf is numbered 1) in each {@see \Vollbehr\Media\Iso14496\Box\Moof},
     *   o trunNumber: indicates the {@see \Vollbehr\Media\Iso14496\Box\Trun} number that contains the random accessible
     *     sample. The number ranges from 1 in each {@see \Vollbehr\Media\Iso14496\Box\Traf}.
     *   o sampleNumber: indicates the sample number that contains the random accessible sample. The number ranges from
     *     1 in each {@see \Vollbehr\Media\Iso14496\Box\Trun}.
     * @return Array
     */
    ...
    *
    ...
    protected function _writeData($writer): void
    {
        ...
        $writer->writeUInt32BE($this->_trackId)
               ->writeUInt32BE(($lengthSizes['trafNum'] << 4) | ($lengthSizes['trunNum'] << 2) | ($lengthSizes['sampleNum']))
     ...
     */
    private function _getLengthSizes(): array
    {
     ...
     */
    private function _getLengthSizeFromInteger($integer): int
    {
        if ($integer <= 0xff) {
            return 0;
        } elseif ($integer <= 0xffff) {
            return 1;
        } elseif ($integer <= 0xffffffff) {
            return 2;
        } else {
            return 3;
        }
